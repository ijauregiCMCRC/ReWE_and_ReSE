...
def make_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    if not model_opt.copy_attn:
        ...
        generator = CopyGenerator(model_opt.rnn_size,
                                  fields["tgt"].vocab)
        if model_opt.ReWE:
            generator_regularizer = CopyGenerator(model_opt.rnn_size,
                                      fields["tgt"].vocab)
    ...
    if checkpoint is not None:
        ...
        if model_opt.ReWE:
            generator_ReWE.load_state_dict(checkpoint['generator_ReWE'])
        if model_opt.ReSE:
            generator_ReSE.load_state_dict(checkpoint['generator_ReSE'])
            #ReSE_gt.load_state_dict(checkpoint['ReSE_gt'])
        ...
        if model_opt.param_init != 0.0:
            ...
            if model_opt.ReWE:
                for p in generator_ReWE.parameters():
                    p.data.uniform_(-model_opt.param_init, model_opt.param_init)
            if model_opt.ReSE:
                for p in generator_ReSE.parameters():
                    p.data.uniform_(-model_opt.param_init,model_opt.param_init)
                # for p in ReSE_gt.parameters():
                #     p.data.uniform_(-model_opt.param_init,model_opt.param_init)
    ...
    if model_opt.ReSE:
        model.generator_ReSE = generator_ReSE
        #model.ReSE_gt = ReSE_gt
