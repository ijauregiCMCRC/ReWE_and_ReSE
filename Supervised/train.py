    ...
    if opt.copy_attn:
        ...
        if opt.ReWE==True and opt.ReSE==False:
            compute = onmt.Loss.NMTLossCompute(
                ...
                label_smoothing=opt.label_smoothing if train else 0.0, ReWE=opt.ReWE,
                generator_ReWE=model.generator_ReWE,dec_embeddings=model.decoder.embeddings,alpha_loss_ReWE=opt.alpha_loss_ReWE, tgt_emb_size=opt.word_vec_size)
        elif opt.ReWE==False and opt.ReSE==True:
            compute = onmt.Loss.NMTLossCompute(
                model.generator, tgt_vocab,
                label_smoothing=opt.label_smoothing if train else 0.0, ReSE=opt.ReSE,
                generator_ReSE=model.generator_ReSE,dec_embeddings=model.decoder.embeddings,alpha_loss_ReSE=opt.alpha_loss_ReSE, tgt_emb_size=opt.word_vec_size)
        elif opt.ReWE==True and opt.ReSE==True:
            compute = onmt.Loss.NMTLossCompute(
                model.generator, tgt_vocab,
                label_smoothing=opt.label_smoothing if train else 0.0, ReWE=opt.ReWE, ReSE=opt.ReSE,
                generator_ReWE=model.generator_ReWE, generator_ReSE=model.generator_ReSE, dec_embeddings=model.decoder.embeddings,
                alpha_loss_ReWE=opt.alpha_loss_ReWE, alpha_loss_ReSE=opt.alpha_loss_ReSE, tgt_emb_size=opt.word_vec_size)
        else:
            compute = onmt.Loss.NMTLossCompute(
                model.generator, tgt_vocab,
...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    if model_opt.neubig_style_training:
        print ("Neubig style training!")
        #Infinite loop
        ...
        while (True):
            ...
            train_iter = make_dataset_iter(lazily_load_dataset("train"),
                                           fields, opt)
            train_stats,no_impr_ppl_num,\
            saved_models,need_to_save,string_saved_model,\
            number_batch,epoch = trainer.train_validate(train_iter,epoch, report_func,number_batch,
                                                 opt.validation_freq_every_X_batch,logger,no_impr_ppl_num,
                                                 saved_models,need_to_save,model_opt,fields,string_saved_model,opt)
        ...
        while(True):
            ...
            # Check if there is improvement, otherwise break
            if valid_stats.ppl() > best_ppl and times_not_impr == 5:
                break
            elif valid_stats.ppl() > best_ppl:
                times_not_impr +=1
            else:
                times_not_impr = 0
                best_ppl = valid_stats.ppl()
...
def main():
    ...
    first_dataset = next(lazily_load_dataset("train"))  #Loads the dataset with the information it had
