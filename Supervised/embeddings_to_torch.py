...
def match_embeddings(vocab, emb, opt):
    ...
    for w, w_id in vocab.stoi.items():
        if w in emb:
            ...
            filtered_embeddings[w_id] = np.random.uniform(-0.1,0.1,dim).tolist()
            count['miss'] += 1
...
def main():
    ...
    parser.add_argument('-emb_file_enc', default='../UnsupervisedMT/NMT/embeddings/en_fr/word/en_fr_word.vec', #required=True,
                        help="source Embeddings from this file")
    parser.add_argument('-emb_file_dec', default='../UnsupervisedMT/NMT/embeddings/en_fr/word/en_fr_word.vec', #required=True,
                        help="target Embeddings from this file")
    parser.add_argument('-output_file', default='IWSLT_2016/french_data/embeddings_transformer/fasttext_word_512',#required=True,
                        help="Output file for the prepared data")
    parser.add_argument('-dict_file', default='../OpenNMT_Last_new/IWSLT_2016/english_french/BASELINE_IWSLT_16_en_fr.TOKENIZED.100l.vocab.pt',#required=True,
                        help="Dictionary file")
    parser.add_argument('-verbose', action="store_true", default=False)
    parser.add_argument('-skip_lines', type=int, default=0,
                        help="Skip first lines of the embedding file")
    parser.add_argument('-type', choices=TYPES, default="word2vec")
