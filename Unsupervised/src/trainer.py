...
class TrainerMT(MultiprocessingEventLoop):
    ...
    def enc_dec_step(self, lang1, lang2, lambda_xe, lambda_ReWE=None, back=False, type_train=None):
        """
        ...
        ReWE_True = False
        # Check ReWE yes or no
        if type_train == 'mono':
            if self.decoder.ReWE_mono:
                ReWE_True = True
        ...
        #Here we specify the loss
        loss_fn = self.decoder.loss_fn[lang2_id]
        if ReWE_True:
            loss_ReWE_fn = self.decoder.loss_ReWE_fn[lang2_id]
        ...
        if ReWE_True:
            scores, scores_ReWE = self.decoder(encoded, sent2[:-1], lang2_id,type_train=type_train)
            xe_loss = loss_fn(scores.view(-1, n_words), sent2[1:].view(-1))
            #Convert word IDs into word embeddings
            unsqueezed_sent2=torch.unsqueeze(sent2[1:].view(-1),0)
            unsqueezed_sent2_embs = self.decoder.embeddings[lang2_id](unsqueezed_sent2)
            sent2_embs=torch.squeeze(unsqueezed_sent2_embs)
            ReWE_loss = loss_ReWE_fn(scores_ReWE.view(-1,self.decoder.emb_dim),sent2_embs,torch.tensor(1, dtype=torch.float).cuda())
        else:
            scores = self.decoder(encoded, sent2[:-1], lang2_id, type_train=type_train)
            xe_loss = loss_fn(scores.view(-1, n_words), sent2[1:].view(-1))
        ...
        assert lambda_xe > 0
        if ReWE_True:
            loss = lambda_xe * (xe_loss + lambda_ReWE * ReWE_loss)
        else:
    ...
    def otf_bt(self, batch, lambda_xe, backprop_temperature, lambda_ReWE=None, lambda_ReSE=None, type_train=None):
        """
        ...
        # Check ReWE yes or no
        if type_train == 'otfd':
            if self.decoder.ReWE_otfd:
                ReWE_True = True
            if self.decoder.ReSE_otfd:
                ReSE_True = True
        ...
        if ReWE_True:
            loss_ReWE_fn = self.decoder.loss_ReWE_fn[lang3_id]
        if ReSE_True:
            loss_ReSE_fn = self.decoder.loss_ReSE_fn[lang3_id]
        ...
        #print (lang3)
        #print (lang3_id)
        if ReWE_True and not (ReSE_True and lang3 == 'en'):
            scores, scores_ReWE = self.decoder(encoded, sent3[:-1], lang_id=lang3_id,type_train=type_train)
            xe_loss = loss_fn(scores.view(-1, n_words3), sent3[1:].view(-1))
            unsqueezed_sent3 = torch.unsqueeze(sent3[1:].view(-1), 0)
            unsqueezed_sent3_embs = self.decoder.embeddings[lang3_id](unsqueezed_sent3)
            sent2_embs = torch.squeeze(unsqueezed_sent3_embs)
            ReWE_loss = loss_ReWE_fn(scores_ReWE.view(-1, self.decoder.emb_dim), sent2_embs,
                                     torch.tensor(1, dtype=torch.float).cuda())
        elif not ReWE_True and ReSE_True and lang3 == 'en':
            scores, scores_ReSE = self.decoder(encoded, sent3[:-1], lang_id=lang3_id,type_train=type_train)
            xe_loss = loss_fn(scores.view(-1, n_words3), sent3[1:].view(-1))
            # ReSE
            sent3_string = self.idSEQ2strSEQ(sent3[1:-1], lang3)
            list_embs = []
            for sen in sent3_string:
            ...
            ReSE_loss = loss_ReSE_fn(scores_ReSE, sen_gt_embs, torch.tensor(1, dtype=torch.float).cuda())

        elif ReWE_True and ReSE_True and lang3 == 'en':
            scores, scores_ReWE, scores_ReSE = self.decoder(encoded, sent3[:-1], lang_id=lang3_id,type_train=type_train)
            ...
            #print (sent3_string)
            if True: #true
                sent3_string = self.process_bpe(sent3_string)
            list_embs = []
            for sen in sent3_string:
                new_embedding = self.sess.run(self.embeddings, feed_dict={self.messages: [sen]})
                new_embedding = torch.from_numpy(new_embedding)
            ...
            scores= self.decoder(encoded, sent3[:-1], lang_id=lang3_id, type_train=type_train)
            xe_loss = loss_fn(scores.view(-1, n_words3), sent3[1:].view(-1))
        self.stats['xe_costs_%s_%s_%s' % direction].append(xe_loss.item())


        assert lambda_xe > 0
        if ReWE_True and not ReSE_True:
            loss = lambda_xe * (xe_loss + lambda_ReWE * ReWE_loss)
        elif not ReWE_True and ReSE_True and lang3 == 'en':
            loss = lambda_xe * (xe_loss + lambda_ReSE * ReSE_loss)
        elif ReWE_True and ReSE_True and lang3 == 'en':
            loss = lambda_xe * (xe_loss + lambda_ReWE * ReWE_loss + lambda_ReSE * ReSE_loss)
        else:
            loss = lambda_xe * xe_loss
    ...
    def idSEQ2strSEQ(self,target_IDs, lang_3):
        list_batch=[]
        sen_len, batch_len = target_IDs.size()
        for i in range(batch_len):
            list_sent = []
            for j in range(sen_len):
                list_sent.append(self.data['dico'][lang_3][int(target_IDs[j,i])])
            list_sent = " ".join(list_sent)
            list_batch.append(list_sent)
        return list_batch

    def process_bpe(self, sent):
        new_sent = []
        for s in sent:
            sen = re.sub(r'(@@ )|(@@ ?$)','', s)
            new_sent.append(sen)
