...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, args, encoder):
        ...
        self.ReWE_mono=args.ReWE_mono
        self.ReWE_otfd=args.ReWE_otfd
        self.ReSE_otfd=args.ReSE_otfd
        ...
        if args.ReWE_mono or args.ReWE_otfd:
            proj_ReWE_layer_1 = [nn.Linear(self.emb_dim,200) for _ in self.n_words]
            proj_ReWE_ReLU = [nn.ReLU() for _ in self.n_words]
            proj_ReWE_layer_2 = [nn.Linear(200, self.emb_dim) for _ in self.n_words]
        if args.ReSE_otfd:
            proj_SelfAttention = [SelfAttention(self.emb_dim) for _ in self.n_words]
            proj_ReSE_layer_1 = [nn.Linear(self.emb_dim,200) for _ in self.n_words]
            proj_ReSE_ReLU = [nn.ReLU() for _ in self.n_words]
            proj_ReSE_layer_2 = [nn.Linear(200, 512) for _ in self.n_words]
        ...
        if args.ReWE_mono or args.ReWE_otfd:
            self.proj_ReWE_l1 = nn.ModuleList(proj_ReWE_layer_1)
            self.proj_ReWE_ReLU = nn.ModuleList(proj_ReWE_ReLU)
            self.proj_ReWE_l2 = nn.ModuleList(proj_ReWE_layer_2)
        if args.ReSE_otfd:
            self.proj_SelfAttention = nn.ModuleList(proj_SelfAttention)
            self.proj_ReSE_l1 = nn.ModuleList(proj_ReSE_layer_1)
            self.proj_ReSE_ReLU = nn.ModuleList(proj_ReSE_ReLU)
            self.proj_ReSE_l2 = nn.ModuleList(proj_ReSE_layer_2)

    def forward(self, encoded, y, lang_id, one_hot=False, incremental_state=None,type_train=None):
        ...
        if type_train=='mono':
            if self.ReWE_mono:
                #project back to size of word embedding
                out_l1 = self.proj_ReWE_l1[lang_id](x)
                out_ReLU = self.proj_ReWE_ReLU[lang_id](out_l1)
                out_l2 = self.proj_ReWE_l2[lang_id](out_ReLU)

                return x_voc, out_l2
        elif type_train == 'otfd':
            if self.ReWE_otfd and not (self.ReSE_otfd and lang_id == 0):
                # project back to size of word embedding
                out_l1 = self.proj_ReWE_l1[lang_id](x)
                out_ReLU = self.proj_ReWE_ReLU[lang_id](out_l1)
                out_l2 = self.proj_ReWE_l2[lang_id](out_ReLU)
                ...
                return x_voc, out_l2
            elif not self.ReWE_otfd and self.ReSE_otfd:
                # project back to size of word embedding
                selfAtt = self.proj_SelfAttention[lang_id](x)
                out_l1 = self.proj_ReSE_l1[lang_id](selfAtt)
                out_ReLU = self.proj_ReSE_ReLU[lang_id](out_l1)
                out_l2 = self.proj_ReSE_l2[lang_id](out_ReLU)

                return x_voc, out_l2
            elif self.ReWE_otfd and self.ReSE_otfd:
                # project back to size of word embedding
                ReWE_out_l1 = self.proj_ReWE_l1[lang_id](x)
                ReWE_out_ReLU = self.proj_ReWE_ReLU[lang_id](ReWE_out_l1)
                ReWE_out_l2 = self.proj_ReWE_l2[lang_id](ReWE_out_ReLU)

                # project back to size of word embedding
                ReSE_selfAtt = self.proj_SelfAttention[lang_id](x)
                ReSE_out_l1 = self.proj_ReSE_l1[lang_id](ReSE_selfAtt)
                ReSE_out_ReLU = self.proj_ReSE_ReLU[lang_id](ReSE_out_l1)
                ReSE_out_l2 = self.proj_ReSE_l2[lang_id](ReSE_out_ReLU)

                return x_voc, ReWE_out_l2, ReSE_out_l2
    ...
    def generate(self, encoded, lang_id, max_len=200, sample=False, temperature=None):
        ...
        while cur_len < max_len:
            ...
            # if self.ReWE:
            #     scores,_ = self.forward(encoded, decoded[:cur_len], lang_id, one_hot, incremental_state)
            #else:
...
class SelfAttention(nn.Module):
    def __init__(self, attention_size):
        ...
        self.attention_1 = nn.Linear(attention_size,500,bias=False)
        self.tanH = nn.Tanh()
        self.attention_2 = nn.Linear(500,1, bias=False)
        self.my_softmax = nn.Softmax(dim=0)

    def forward(self,x_in):
        # after this, we have (batch,dim1) with a diff weight per each cell
        #attention_score = torch.matmul(x_in,self.attention).squeeze(2)
        attention_score = self.attention_1(x_in)
        attention_score = self.tanH(attention_score)
        attention_score = self.attention_2(attention_score).squeeze(2)
        #print (attention_score.size())

        attention_score = self.my_softmax(attention_score)
        attention_score= attention_score.unsqueeze(2)
        scored_x = x_in * attention_score

        #now, sum across dim 1 to get the expected feature vector
        condensed_x = torch.sum(scored_x,dim=0)
        #print (condensed_x.size())
