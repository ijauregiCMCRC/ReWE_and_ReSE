...
def build_attention_model(params, data, cuda=True):
    ...
    #ReWE: Regressing Word Embeddings (if at least one of the ReWE's is true)
    if params.ReWE_mono or params.ReWE_otfd:
        loss_ReWE_fn = []
        for _ in params.n_words:
            loss_ReWE_fn.append(nn.CosineEmbeddingLoss(size_average=True))
        decoder.loss_ReWE_fn = nn.ModuleList(loss_ReWE_fn)
    if params.ReSE_otfd:
        loss_ReSE_fn = []
        for _ in params.n_words:
            loss_ReSE_fn.append(nn.CosineEmbeddingLoss(size_average=True))
        decoder.loss_ReSE_fn = nn.ModuleList(loss_ReSE_fn)
